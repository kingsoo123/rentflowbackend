...
- `POST /api/device-push-token` — **any authenticated** JWT (`tenant` or `property_manager`). JSON `{ token, platform }` with `platform` **`ios`** | **`android`** (native APNs / FCM device token from `expo-notifications`). Upserts **`user_device_push_tokens`** on `(user_id, token)`. Requires migration **`CreateUserDevicePushTokens1762100000000`**. **FCM delivery** (Firebase Admin) uses **Android** tokens only when Firebase is configured via **`FIREBASE_SERVICE_ACCOUNT_BASE64`**, **`FIREBASE_SERVICE_ACCOUNT_JSON`**, **`FIREBASE_SERVICE_ACCOUNT_PATH`** (path to the downloaded service account JSON), or **`GOOGLE_APPLICATION_CREDENTIALS`**; pushes fire on rent renewal notices, maintenance status alerts, and portfolio broadcasts.
