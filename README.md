...
Default HTTP port is **3002** (Next.js uses **3000** and often **3001** when 3000 is busy). Override with **`PORT`**. Set `CORS_ORIGIN` to your frontend origin (for example `http://localhost:3000`).
...
- `POST /api/tenants/maintenance-requests` — **tenant** JWT only; JSON `{ title, description, urgency, attachmentUrls? }` (`urgency`: `low` | `normal` | `high` | `emergency`; optional `attachmentUrls`: up to 10 HTTPS URLs). Requires migration `CreateMaintenanceRequestsTable1761100000000` (`npm run typeorm:migration:run`). On success, connected property managers receive Socket.IO event `maintenance:created` on namespace `/managers/maintenance` (see below).
- `POST /api/tenants/assistant/chat` — **tenant** JWT only; JSON `{ message }` (trimmed, 1–2000 chars). Returns `{ reply }` with guidance that references current tenant features and API routes (maintenance, notifications, upcoming rent, **GET /api/tenants/profile** for unit/building, manager onboarding **POST /api/managers/tenants**, Socket.IO `/tenants/notifications`, etc.).
...
- `GET /api/tenants/profile` — **tenant** JWT only; returns `{ unitNumber, propertyAssigned }` from `tenant_profiles` (manager onboarding via `POST /api/managers/tenants`), for dashboard header copy.
...
- `GET /api/managers/tenants` — **property manager** JWT only; paginated directory (`page`, `limit`, optional `search`, optional `property` fuzzy name filter, optional **`propertyId`** UUID). Use **`propertyId`** alone to list tenants for one portfolio building (must belong to you; **404** if unknown). Do not send both **`property`** and **`propertyId`** (**400**). **Only includes tenants** whose `tenant_profiles.profile_data->>'propertyAssigned'` (trimmed, compared case-insensitively) **matches one of your `properties` names** from signup (`properties.manager_user_id` = JWT `sub`), or when `propertyId` is set, **exactly matches that property’s name**. Managers with no properties see an empty list. Each list item may include **`renewalEffectiveDate`**, **`renewalMonthlyRentDisplay`**, **`rentPaymentStatusLabel`**, **`serviceChargePaymentStatusLabel`** (renewal from latest delivered rent renewal notification; labels from profile / heuristics / published service charge lines).
...
- `GET /api/managers/maintenance-requests` — **property manager** JWT only; lists tenant-submitted maintenance (newest first), including tenant name and email for triage.
- `PATCH /api/managers/maintenance-requests/:id` — **property manager** JWT only; JSON `{ status }` where `status` is `submitted` | `reviewing` | `in_progress` | `resolved`. When the status **changes**, creates a tenant in-app notification (`kind: maintenance_status`) and emits **`notifications:updated`** on **`/tenants/notifications`** so the tenant **Alerts** list refreshes.
- **WebSocket (Socket.IO)** — namespaces (same host/port as the HTTP API, e.g. `http://localhost:3002/...`):
  - **`/managers/maintenance`** — connect with a **property manager** JWT in `handshake.auth.token` (or query `token`). Server joins the client to room `managers-maintenance` and emits **`maintenance:created`** with `{ id: string }` when a tenant creates a request.
  - **`/tenants/notifications`** — connect with a **tenant** JWT in `handshake.auth.token` (or query `token`). Server joins the client to a per-tenant room and emits **`notifications:updated`** (payload `{ id?: string }`) when a new in-app notification is created (e.g. rent renewal from a property manager, or a **maintenance status** update after `PATCH /api/managers/maintenance-requests/:id`).
