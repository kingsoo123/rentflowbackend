...
Default HTTP port is **3002** (avoids clashes when Next.js uses **3000** and sometimes **3001**). Override with `PORT` in `.env`. Set `CORS_ORIGIN` to your frontend origin (for example `http://localhost:3000`).
...
- `POST /api/auth/signup` — register with JSON body `{ name, role, email, password, confirmPassword, terms }` (`role`: `property_manager` | `tenant` | `artisan`, same as the EstateFlow signup form). For **`property_manager`**, include **`propertyNames`**: a string of comma-separated property names (trimmed, deduped case-insensitively server-side; each name ≤200 chars). Rows are stored in **`properties`** with a unique constraint per manager on **`lower(trim(name))`** so each property is a distinct record for assigning tenants later.
...
- `POST /api/device-push-token` — **any authenticated** JWT (`tenant` or `property_manager`). JSON `{ token, platform, tokenProvider? }` with `platform` **`ios`** | **`android`**, optional **`tokenProvider`**: omit or **`native`** for APNs/FCM device tokens from `expo-notifications`; **`expo`** for `ExponentPushToken[...]` from `Notifications.getExpoPushTokenAsync` (typical for **Expo builds on iOS**). Upserts **`user_device_push_tokens`**. Run migrations through **`1762200000000-AddTokenProviderToUserDevicePushTokens`**. **Native FCM** uses Android rows with `token_provider: native` when Firebase is configured (**`FIREBASE_SERVICE_ACCOUNT_BASE64`**, **`FIREBASE_SERVICE_ACCOUNT_JSON`**, **`FIREBASE_SERVICE_ACCOUNT_PATH`**, or **`GOOGLE_APPLICATION_CREDENTIALS`**). **Expo** tokens are sent via Expo’s HTTP push API. Pushes fire on rent renewal notices, maintenance status alerts, portfolio broadcasts, **service charge** updates, **manager task** alerts from the mobile manager app, and **lease form** submissions from tenants (to rostered property managers).
...
- `POST /api/tenants/maintenance-requests/upload` — **tenant** JWT only; `multipart/form-data` with field **`file`** (one image, ≤5MB). Saves under `uploads/maintenance/` and returns `{ path, url }` where `path` is `/api/uploads/maintenance/:filename`. **Download** via authenticated `GET /api/uploads/maintenance/:filename` (tenant JWT for own requests, property manager JWT for roster tenants). Optional env **`PUBLIC_BASE_URL`** (no trailing slash) overrides the absolute `url` when behind a proxy. Files in `uploads/` are gitignored.
...
- `GET /api/tenants/lease-forms/templates` — **tenant** JWT only; returns an array of form definitions (`slug`, `title`, `intro`, `fields`, `signaturePrompt`) for the resident acknowledgment flow.
- `GET /api/tenants/lease-forms/my-submissions` — **tenant** JWT only; returns `{ id, formSlug, submittedAt }[]` (newest first, max 50).
- `POST /api/tenants/lease-forms/submit` — **tenant** JWT only; JSON `{ formSlug, answers, signatureText }`. Validates required fields, stores **`tenant_lease_form_submissions`**, emits **`lease_form:submitted`** on **`/managers/maintenance`** to each rostered manager, and sends **push** to those managers. Requires migration **`1762300000000-CreateTenantLeaseFormSubmissions`** and dependency **`pdfkit`** (`npm install`).
- `GET /api/tenants/lease-forms/submissions/:id/pdf` — **tenant** JWT only; returns **`application/pdf`** of the submission (answers + typed electronic signature). **404** if the submission is not yours.
- `GET /api/managers/lease-form-submissions` — **property manager** JWT only; lists recent submissions from tenants on your occupancy roster (`id`, `tenantId`, `tenantName`, `tenantEmail`, `formSlug`, `submittedAt`).
- `GET /api/managers/lease-form-submissions/:id/pdf` — **property manager** JWT only; same PDF as the tenant download for submissions tied to your roster (**404** otherwise).
...
- `PUT /api/managers/properties/:propertyId/service-charges` — **property manager** JWT only; JSON `{ lines: [{ label, amount }] }` (max **40** lines; `label` 1–200 chars; `amount` ≥ 0, ≤ 2 decimal places). Replaces all rows for the property; tenants with that **property name** on file see them on **`GET /api/tenants/service-charges`**. Omit blank rows; send `{ lines: [] }` to clear. After a successful save, creates a **`service_charges`** in-app notification and **push** (Expo + native FCM) per affected tenant, emits **`notifications:updated`**, and emits **`service-charges:updated`** on **`/tenants/notifications`** so dashboards refetch line items immediately.
...
- `GET /api/managers/artisans` — **property manager** JWT only; workers / artisans on **your roster** (newest first), with trade and property from roster `profile_data`.
- `GET /api/managers/artisans/lookup-email` — **property manager** JWT only; query `email` — returns `{ existsAsArtisan, hasAccount }` (worker must sign up with role **`artisan`** first).
- `POST /api/managers/artisans` — **property manager** JWT; JSON `{ name, email, profile? }` links an existing **`artisan`** account by login email to your roster (`manager_artisan_roster`). Optional `profile.propertiesAssigned` must match one of your **`properties`** names. Returns the user row; **200** if roster row updated, **201** if newly linked. Requires migration `CreateManagerArtisanRoster1762900000000`.

### Workers / artisans (JWT role `artisan`)

- `GET /api/artisans/profile` — **artisan** JWT; account summary, manager roster entries, assigned property names, and `activeMaintenanceCount`.
- `GET /api/artisans/assignments` — **artisan** JWT; pending and accepted maintenance assignments (accept within **2 hours** of `assignedAt`).
- `POST /api/artisans/assignments/:id/accept` — **artisan** JWT; accept a pending assignment before `acceptBy` expires.
- `GET /api/artisans/maintenance-requests` — **artisan** JWT; legacy property-scoped maintenance list (superseded by assignments for assigned work).

### Maintenance worker assignments (manager + artisan)

- `POST /api/managers/maintenance-requests/:id/assign` — **property manager** JWT; JSON `{ artisanUserId }` assigns a roster worker; worker has **2 hours** to accept (`acceptBy`). Returns updated maintenance row with `assignment`. Requires migration `CreateMaintenanceAssignments1763000000000`.
...
- `POST /api/managers/tasks/notify-tenants` — **property manager** JWT only; JSON `{ title, description?, dueLabel?, tenantIds? }`. If **`tenantIds`** is omitted, all tenants on your occupancy roster are notified (max **200**). Creates in-app rows (`kind: manager_task`), emits **`notifications:updated`**, and sends push (Expo + native Android FCM as configured). Intended when a manager adds a task in the mobile app.
...
- **WebSocket (Socket.IO)** — namespaces (same host/port as the HTTP API, e.g. `http://localhost:3002/...`):
  - **`/managers/maintenance`** — connect with a **property manager** JWT in `handshake.auth.token` (or query `token`). Server joins the client to a **per-manager room** (`manager:<JWT sub>`) and emits **`maintenance:created`** with `{ id: string }` when a tenant on that manager’s occupancy roster creates a request. Also emits **`lease_form:submitted`** with `{ id, tenantId, tenantName }` when a tenant submits the resident lease acknowledgment form (`POST /api/tenants/lease-forms/submit`).
  - **`/tenants/notifications`** — connect with a **tenant** JWT in `handshake.auth.token` (or query `token`). Server joins the client to a per-tenant room and emits **`notifications:updated`** (payload `{ id?: string }`) when a new in-app notification is created (e.g. rent renewal, maintenance status alert, property broadcast, **service charges save**, manager task). Emits **`maintenance:updated`** with `{ id: string }` when a property manager changes a maintenance request status (`PATCH /api/managers/maintenance-requests/:id`) so tenant clients can refresh maintenance rows without waiting for the notification write. Emits **`service-charges:updated`** (payload `{}`) when a property manager saves service charges (`PUT /api/managers/properties/:propertyId/service-charges`) so tenant dashboards can refetch **`GET /api/tenants/service-charges`** immediately.
