...
export class TenantPaymentConfirmationsService {
  ...
  async listRentArrearsForManager(managerUserId: string): Promise<{
    ...
    try {
      ...
      await Promise.all(
        rosterIds.map(async (tenantId) => {
          ...
          const serviceChargeDue =
            serviceCharges.source === 'active' && serviceCharges.lines.length > 0
              ? serviceCharges.lines.reduce(
                  (acc, line) =>
                    acc + (Number.isFinite(line.amount) ? line.amount : 0),
                  0,
                )
              : 0;
          ...
          const daysLate = Math.floor(
            (todayUtc - dueDate.getTime()) / 86_400_000,
          );
          if (daysLate < 1) {
            return;
          }
          ...
          rows.push({
            tenantId,
            tenantName:
              user?.fullName?.trim() || user?.email?.trim() || 'Tenant',
      ...
      this.rethrowIfMissingTable(err);
      this.logger.error(
        'listRentArrearsForManager failed',
        err instanceof Error ? err.stack : String(err),
      );
      throw new InternalServerErrorException('Could not load rent arrears.');
    }
  }
